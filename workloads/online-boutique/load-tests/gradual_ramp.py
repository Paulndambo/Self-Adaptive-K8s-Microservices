...
def build_steps(
    ...
    for index in range(step_count):
        ...
        users = peak_users if step_count == 1 else round(initial_users + ((peak_users - initial_users) * (index / (step_count - 1))))
        steps.append(LoadStep(duration_seconds=step_duration, users=max(1, users)))
    return steps


def main() -> None:
    parser = argparse.ArgumentParser(description="Run a gradual-ramp Online Boutique workload.")
    ...
    summary = run_load_test(
        scenario="online_boutique_gradual_ramp",
        base_url=args.base_url,
        steps=build_steps(args.duration_seconds, args.initial_users, args.peak_users, args.ramp_step_seconds),
